...
exports.handler = sentrify(async (event, context, callback) => {
  ...
  try {
    ...
    const data = JSON.parse(
      (await getFromS3("jude-io-transactions", event.key)).Body.toString()
    );
    ...
    for (let trans of items) {
      ...
      if (trans._jude.startsWith("transfer_")) {
        ...
        result = await markTransferAsReceived(
          trans._jude,
          trans._id,
          trans._user
        );
      ...
      if (result.alreadyMarked) {
        ...
        Log(
          "handler",
          "transfer/payment is already marked as received",
          trans._jude
        );
        continue;
      }
      ...
      if (trans.notify && result.payee) {
        ...
        if (device.push_token) {
          ...
          if (trans._jude.startsWith("payment_")) {
            ...
            message =
              `Payment of ${numeral(trans.amount).format("$0,000.00")} from ${
                payer.first_name
              } has arrived ` + `in your ${to.name} account`;
          } else {
            const to = JudeUsers.getAccount(user._id, result.to);
            message =
              `Transfer of ${numeral(trans.amount).format(
                "$0,000.00"
              )} has arrived in your ` + `${to.name} account`;
          }
